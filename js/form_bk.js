...
$("#js-to-step03").on("click", function () {
    ...
    var sc_pos = $("#js-form-step03").offset().top; //スクロールポジション取得


    /* フォーム擬似バリデーション */
    $('#zip,#address,#name,#namekana,#year,#day,#date,#month,#line_id,#time01,#time02,#sex,#mail,#tel').each(function () {
        var input_name = $(this).attr("name");
        if ($(this).val() === '') {
            alert(input_name + 'を入力してください');
            $(this).focus();
            isEmpty = true;
            return false;
        }
...
$("input:radio[name='エリア選択']").on("change", function () {
    ...
    if ($("[value='北海道エリア']").prop("checked")) {
        window.open(
            "https://connect-app.net/r_yag/reserve/main?cid=5",
            "_blank"
        );
    }
    ...
    if ($("[value='関東エリア']").prop("checked")) {
        window.open(
            "https://connect-app.net/r_yag/reserve/main?cid=2",
            "_blank"
        );
    }
    ...
    if ($("[value='関西エリア']").prop("checked")) {
        window.open(
            "https://connect-app.net/r_yag/reserve/main?cid=1",
            "_blank"
        );
    }
    ...
    if ($("[value='九州エリア']").prop("checked")) {
        window.open(
            "https://connect-app.net/r_yag/reserve/main?cid=6",
            "_blank"
        );
    }
